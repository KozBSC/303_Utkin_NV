...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "SELECT DISTINCT t1.name, t2.name, t1.title FROM (SELECT u.name, m.title FROM ratings r INNER JOIN users u ON r.user_id = u.id INNER JOIN  movies m  ON r.movie_id = m.id) AS t1 JOIN (SELECT u.name, m.title FROM ratings r INNER JOIN users u ON r.user_id = u.id INNER JOIN  movies m  ON r.movie_id = m.id) AS t2 ON t1.title=t2.title WHERE t1.name<t2.name ORDER BY t1.title, t1.name, t2.name"
echo " "
